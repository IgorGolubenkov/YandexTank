...
def get_1(*args, **kwargs):
    try:
        ...
        answ = requests.post(url=req, data=json.dumps(payload), headers=headers, verify=False)
...
def get_2(*args, **kwargs):
    try:
        ...
        answ = requests.post(url=req, data=json.dumps(payload), headers=headers_auth, verify=False)
        if answ.status_code != 200:
            logger.error('Not 200 answer code for Req: %s \n Payload: %s , Response: %s' % (req, payload, answ.json()))
            raise HttpCode(answ.status_code)
...
def get_4(*args, **kwargs):
    try:
        ...
        answ = requests.get(url=req, headers=headers_token, verify=False)
        if answ.status_code != 200:
...
def get_5(*args, **kwargs):
    try:
        ...
        logger.debug('Attempt a post request oauth: %s, payload: %s' % (req, payload))
        answ = requests.post(url=req, data=json.dumps(payload), headers=headers_oauth, verify=False)
        if answ.status_code != 200:
            logger.error('Not 200 answer code for Req: %s \n Payload: %s , Response: %s' % (req, payload, answ.json()))
            raise HttpCode(answ.status_code)
...
def get_7(*args, **kwargs):
    try:
        ...
        logger.debug('Attempt a get request: %s' % (req))
        answ = requests.get(url=req, headers=headers_oauth, verify=False)
        if answ.status_code != 200:
            logger.error('Not 200 answer code for Req: %s \n Response: %s' % (req, answ.json()))
            raise HttpCode(answ.status_code)
...
def get_8(*args, **kwargs):
    try:
        req = '%s/api/leases' % test_case['url2']
        session_token = test_results['result6']
        x_account = test_results['result4']
        extension_leases = {
            "extend": "catalog,keys"
        ...
        logger.debug('Attempt a get request: %s' % (req))
        answ = requests.get(url=req, headers=headers_oauth, params=extension_leases, verify=False)
        if answ.status_code != 200:
            logger.error('Not 200 answer code for Req: %s \n Response: %s' % (req, answ.json()))
            raise HttpCode(answ.status_code)
...
def get_10(*args, **kwargs):
    try:
        req = '%s/api/libraries' % test_case['url2']
        extension_leases = {
            "extend": "catalog,keys"
        }
        session_token = test_results['result6']
        ...
        logger.debug('Attempt a get request: %s' % (req))
        answ = requests.get(url=req, params=extension_leases, headers=headers_oauth, verify=False)
        if answ.status_code != 200:
            logger.error('Not 200 answer code for Req: %s \n Response: %s' % (req, answ.json()))
            raise HttpCode(answ.status_code)
...
def get_9(*args, **kwargs):
    try:
        ...
        create_libraries_body = {
            'owner': x_account,
            'name': 'yandex tank test',
        }
        logger.debug('Attempt a get request: %s' % (req))
        answ = requests.post(url=req, headers=headers_oauth, json=create_libraries_body, verify=False)
        if answ.status_code != 201:
            logger.error('Not 201 answer code for Req: %s \n Response: %s' % (req, answ.json()))
            raise HttpCode(answ.status_code)
        logger.debug('Response Create Libraries: %s' % answ.json())
        return answ.json()['id']
    except RuntimeError as e:
        logger.Error('Scenario failed with %s' % e)
...
def get_11(*args, **kwargs):
    try:
        req = '%s/api/libraries/%s' % (test_case['url2'], test_results['result9'])
        session_token = test_results['result6']
        x_account = test_results['result4']
        headers_oauth = {
            "X-Access-Token": session_token,
        ...
        if answ.status_code != 204:
            ...
            raise HttpCode(answ.status_code)
        logger.debug('Response Delete Libraries: %s' % answ.headers)
        return "OK"
    except RuntimeError as e:
        logger.Error('Scenario failed with %s' % e)
...
def scenario1(missile, marker, results):
    try:
        ...
        with measure("create_libraries", results):
            test_results['result9'] = get_9(test_case, test_results)
        with measure("get_libraries", results):
            test_results['result10'] = get_10(test_case, test_results)
        with measure("delete_libraries", results):
